	...
	for record in records:
		...
		try:
			...
			if json_topics:
				...
				assign_label_json(dict(zip(json_topics_clean,json_topics)), graph)
...
def getDBpediaThumbnail(topic: str) -> str:
	...
	result = requests.get("https://dbpedia.org/sparql?default-graph-uri=http%3A%2F%2Fdbpedia.org&format=json&query=" + quote(query))
...
def main(argv):
	...
	if os.path.isfile("publicationsGraphWithoutImages.rdf"):
